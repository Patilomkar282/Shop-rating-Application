...
import { LayoutDashboard, Users, KeyRound, LogOut, Store as StoreIcon } from 'lucide-react';

export default function OwnerLayout() {
  const location = useLocation();
  const navigate = useNavigate();
  const userName = localStorage.getItem('name') || 'Store Owner';
  ...
  const navItems = [
    ...
    { name: 'Update Password', path: '/owner/security', icon: <KeyRound size={20} /> },
  ];
      ...
      <aside className="w-64 bg-white border-r border-slate-200 flex flex-col fixed h-full shadow-sm">
        <div className="p-6 border-b border-slate-100">
          <div className="flex items-center gap-3 mb-2">
            <div className="w-10 h-10 bg-emerald-100 text-emerald-600 rounded-xl flex items-center justify-center font-bold shadow-inner">
              <StoreIcon size={20} />
            </div>
            <div>
              <h2 className="text-lg font-bold text-slate-900 leading-tight truncate w-36">{userName}</h2>
              <p className="text-xs text-emerald-600 font-bold uppercase tracking-wider">Store Owner</p>
            </div>
          </div>
        </div>
